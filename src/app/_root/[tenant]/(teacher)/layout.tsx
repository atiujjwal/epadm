...
export default async function TeacherLayout({ children }: Props) {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#f0f7ff_0%,#f8fafc_48%,#ffffff_100%)]">
      <div className="mx-auto flex min-h-screen w-full max-w-[1600px] gap-0">
        <aside className="hidden w-72 border-r border-zinc-200/80 bg-white/80 px-5 py-6 backdrop-blur md:block">
          ...
          <nav className="mt-6 space-y-1">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex items-center rounded-xl px-3 py-2.5 text-sm font-medium text-zinc-700 transition hover:bg-sky-50 hover:text-sky-800"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="mt-8 rounded-2xl border border-zinc-200 bg-sky-50/50 px-4 py-4 text-sm text-zinc-700">
