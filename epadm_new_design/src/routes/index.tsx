...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "EPADM — AI School Operating System" },
      {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="sticky top-0 z-30 border-b bg-background/80 backdrop-blur">
        <div className="mx-auto max-w-7xl px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <a href="#login" className="text-[12px] text-muted-foreground hover:text-foreground">Sign in</a>
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,theme(colors.accent/0.4),transparent_60%)]" />
        <div className="relative mx-auto max-w-7xl px-6 pt-16 pb-20 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              <Button asChild className="h-10 text-[13px]"><a href="#login">Sign in to your school <ArrowRight className="h-4 w-4 ml-1.5" /></a></Button>
              <a href="#platform" className="text-[13px] text-muted-foreground hover:text-foreground">Explore the platform →</a>
            </div>
            <div className="mt-10 grid grid-cols-3 gap-6 max-w-lg">
              {[
                ...
                <div key={l}>
                  <div className="text-[20px] font-semibold font-mono tabular">{v}</div>
            ...
            <div className="rounded-lg border bg-surface shadow-lg p-6">
              ...
              <form className="mt-5 space-y-3" onSubmit={(e) => e.preventDefault()}>
                <div>
                  ...
                  <Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@school.edu" className="h-9 mt-1 text-[13px]" />
                </div>
                <div>
                  <div className="flex items-center justify-between">
                    <label className="text-[11px] font-medium text-muted-foreground">Password</label>
                    <a href="#" className="text-[10px] text-muted-foreground hover:text-foreground">Forgot?</a>
                  </div>
                  <Input value={password} onChange={(e) => setPassword(e.target.value)} type="password" placeholder="••••••••" className="h-9 mt-1 text-[13px]" />
                </div>
                <Button asChild className="w-full h-9 text-[13px] mt-2">
                  <Link to="/dashboard">Continue</Link>
                </Button>
                <div className="relative py-1">
                  <div className="absolute inset-x-0 top-1/2 h-px bg-border" />
                  <div className="relative text-center text-[10px] uppercase tracking-wider text-muted-foreground bg-surface w-fit mx-auto px-2">or</div>
                </div>
                <Button variant="outline" className="w-full h-9 text-[13px]">Sign in with SSO</Button>
              </form>
        ...
        <div className="mx-auto max-w-7xl px-6 py-20 grid lg:grid-cols-2 gap-12 items-start">
          <div>
            ...
            <div className="mt-6 flex gap-3">
              <Button asChild className="h-9 text-[13px]"><Link to="/ai-studio">Explore AI Studio <ArrowRight className="h-4 w-4 ml-1.5" /></Link></Button>
            </div>
          </div>
          ...
          <div className="mt-10 grid md:grid-cols-3 gap-4">
            {plans.map((p) => (
              <div key={p.name} className={`rounded-lg border bg-surface p-6 ${p.featured ? "ring-2 ring-primary" : ""}`}>
                {p.featured && <div className="text-[10px] uppercase tracking-wider text-primary font-medium mb-2">Most popular</div>}
                <div className="text-[15px] font-semibold">{p.name}</div>
                <div className="text-[11px] text-muted-foreground mt-0.5">{p.desc}</div>
                <div className="mt-4 flex items-baseline gap-1">
                  <span className="text-[28px] font-semibold tabular font-mono">{p.price}</span>
                  {p.price !== "Custom" && <span className="text-[12px] text-muted-foreground">/ student / mo</span>}
                </div>
                ...
                <Button className="w-full mt-6 h-9 text-[13px]" variant={p.featured ? "default" : "outline"}>Talk to us</Button>
              </div>
            ))}
          </div>
        </div>
